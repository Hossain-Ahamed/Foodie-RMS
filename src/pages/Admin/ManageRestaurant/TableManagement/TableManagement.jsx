...
const TableManagement = () => {
  ...
  const {
    ...
    isLoading: existingTableLoading,
    error: existingTableError,
  ...
  const deleteTableMutation = useMutation(
    async (tableNo) => {
      const res = await axiosSecure.delete(
        `/restaurant/${res_id}/branch/${branchID}/tables/${tableNo}`
      );
      return res?.data;
    },
    {
  ...
  const onSubmit = async (formData) => {
    const existingTableIndex = existingTableData.findIndex(
      (table) => table?.number.toString() === formData?.number
    );
    ...
    if (existingTableIndex !== -1) {
      ...
      await addTableMutation.mutateAsync(formData);
      reset();
      ...
      <>
        <div className="w-full flex justify-end pr-6">
          <Button
            ...
            variant="solid"
            onPress={onOpen}
            endContent={<IoAddOutline className="text-white" />}
          >
                  ...
                  <form onSubmit={handleSubmit(onSubmit)}>
                    <div className="w-full  p-3">
                      ...
                      {errors.number?.type === "required" && (
                        <p
                          className="m-0 p-0 pl-1  text-base text-red-500 text-[9px]"
                          role="alert"
                        >
                          {errors?.number?.message}
                        </p>
                      )}
                      {errors.number?.type === "isNumber" && (
                        <p
                          className="m-0 p-0 pl-1  text-base text-red-500 text-[9px]"
                          role="alert"
                        >
      ...
      <div className="mt-8">
        <div className="flex flex-wrap gap-3">
          {existingTableData.map((table, _idx) => (
            <div
              key={_idx}
              className="relative max-w-80 flex flex-col items-center rounded-lg p-4 border-slate-950 shadow-sm shadow-indigo-300"
            >
              <QRCode
                value={table?.qrCodeData}
                id={table?.qrCodeData + _idx + table?.number}
              ...
              <div className="mt-2">
                ...
                >
                  <a href={table?.qrCodeData}>d</a>
                  {table?.qrCodeData.slice(0, 40)}
                </p>
                {table?.qrCodeData.length > 40 && (
                  <Tooltip id={_idx + table?.qrCodeData} />
                )}
                ...
                <div className=" absolute top-0 right-5 flex flex-col gap-5 border-t pt-4">
                  <div className="inline-flex self-center items-center">
                    <button
                      ...
                      className="inline-flex self-center items-center p-2 text-sm font-medium text-center text-gray-900 bg-gray-200 rounded-lg hover:bg-gray-100 focus:ring-4 focus:outline-none  focus:ring-gray-50 "
                      type="button"
                    >
                      <svg
                        className="w-4 h-4 text-gray-900 "
