 ...
 */
    ...
    */
    ...
    const { data: responseData = {}, isLoading: queryLoading, error: queryError, refetch } = useQuery({
        ...
        queryFn: async () => {
            const res = await axiosSecure.get(`/admin/restaurant/${res_id}/branch/${branchID}/all-orders-list?search=${searchValue}&currentPage=${currentPage - 1}&numberOfSizeInTableData=${numberOfSizeInTableData}&typeOfRange=${typeOfRange}&startingDate=${startingDate}&endingDate=${EndingDate}`)
    ...
    return (
        <div className='container mx-auto px-4 sm:px-8 select-none'>
            <SetTitle title="All Order" />
            <SectionTitle h1='All Orders' />
                ...
                <div className='-mx-4 sm:-mx-8 px-4 sm:px-8 py-4 overflow-x-auto'>
                    <div className='inline-block min-w-full shadow rounded-lg overflow-hidden'>
                        ...
                        <table className='min-w-full leading-normal'>
                            ...
                            <tbody>{DataArrayList && Array.isArray(DataArrayList) && DataArrayList.map((order, _idx) => <AllOrderTableRow key={_idx} order={order} PaymentType={responseData?.paymentTypes} refetch={refetch}/>)}</tbody>
                        </table>
