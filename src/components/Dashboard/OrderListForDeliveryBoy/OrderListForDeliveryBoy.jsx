...
import OrderListForDeliveryBoyTableRow from "./OrderListForDeliveryBoyTableRow";
import useProfile from "../../../Hooks/useProfile";
import LoadingPage from "../../../pages/Shared/LoadingPages/LoadingPage/LoadingPage";
const OrderListForDeliveryBoy = () => {
 ...
 */
    ...
    */
    ...
    const { data: responseData = {}, isLoading: queryLoading, error: queryError, refetch } = useQuery({
        ...
        enabled : (!profileLoading && (!!profile?._id)),
        queryFn: async () => {
           
            const res = await axiosSecure.get(`/admin/restaurant/${res_id}/branch/${branchID}/all-orders-list/delivery-boy/${profile?._id}?search=${searchValue}&currentPage=${currentPage - 1}&numberOfSizeInTableData=${numberOfSizeInTableData}&typeOfRange=${typeOfRange}&startingDate=${startingDate}&endingDate=${EndingDate}`)
            ...
            <div className='py-8'>
                <div className='-mx-4 sm:-mx-8 px-4 sm:px-8 py-4 overflow-x-auto'>
                    <div className='inline-block min-w-full shadow rounded-lg overflow-hidden'>
                        {
                            (queryLoading || profileLoading) && <TableLoading />
                        ...
                        <table className='min-w-full leading-normal'>
                            ...
                            <tbody>{DataArrayList && Array.isArray(DataArrayList) && DataArrayList.map((order, _idx) => <OrderListForDeliveryBoyTableRow key={_idx} order={order} PaymentType={responseData?.paymentTypes} refetch={refetch}/>)}</tbody>
                        </table>
...
export default OrderListForDeliveryBoy